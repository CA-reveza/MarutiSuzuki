...
router.post("/api/order", async (req, res) => {
  ...
  const name = user.name || body.customerName || "Maruti Suzuki Guest";
  const phone = user.phone || body.customerPhone || "+91 98201 00000";
  const email = user.email || body.customerEmail || "guest@marutisuzukis.com";
  ...
  const orderId = body.orderId || shortId("MS-ORD");
  const order = {
    id: orderId,
    order_id: orderId,
    customer_name: name,
