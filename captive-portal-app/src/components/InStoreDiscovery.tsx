...
import marutiSuzukiLogo from '../assets/marutisuzuki-logo.png';
...
export const InStoreDiscovery: React.FC<InStoreDiscoveryProps> = ({ customer, onViewVoucher, onLogout }) => {
  ...
  const [activeSection, setActiveSection] = useState<'sales' | 'services'>('sales');
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        const alreadyChecked = sessionStorage.getItem('marutisuzuki_portal_pulse_checked') === 'true';
        if (!alreadyChecked) {
          setReviewModalConfig({
            isOpen: true,
            title: "In-Store Wi-Fi & Shopping Check",
            subtitle: "Is everything going smoothly during your visit to Maruti Suzuki?",
            badgeText: "In-Store Quality Check",
            onComplete: () => {
              try {
                sessionStorage.setItem('marutisuzuki_portal_pulse_checked', 'true');
              } catch (e) {}
            }
          });
        }
  ...
  const handleInitiateLogout = () => {
    setReviewModalConfig({
      isOpen: true,
      title: "How was your overall Maruti Suzuki In-Store & Wi-Fi Experience?",
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] text-[#1a1a1a] pb-24">
      ...
      <header className="sticky top-0 z-40 bg-[#ffffff]/95 backdrop-blur-md border-b border-[#e8e2d5] shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2.5">
              <img src={marutiSuzukiLogo} alt="Maruti Suzuki" className="h-9 sm:h-10 w-auto" />
        ...
        {/* SALES / SERVICES TOP NAVIGATION */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-3">
          <div className="inline-flex items-center gap-1 p-1 bg-[#faf8f5] border border-[#e8e2d5] rounded-full">
            <button
              onClick={() => setActiveSection('sales')}
              className={`px-4 sm:px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer flex items-center gap-1.5 ${
                activeSection === 'sales'
                  ? 'bg-[#122B52] text-white shadow-xs'
                  : 'text-[#555045] hover:text-[#1a1a1a]'
              }`}
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-10 space-y-12 sm:space-y-16">
        ...
        <section className="space-y-6">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#122B52]/10 text-[#122B52] text-xs font-bold uppercase tracking-widest mb-3">
              ...
              <span>JUST IN • ON THE SALES FLOOR</span>
            </div>
            ...
            <p className="text-sm sm:text-base text-[#555045] mt-2 leading-relaxed">
              "Discover the latest Maruti Suzuki models, offers, and test drive slots available at this showroom."
            </p>
          </div>
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6 sm:p-10 text-white">
              <div className="max-w-xl space-y-2">
                ...
                <h2 className="text-2xl sm:text-4xl font-serif font-bold">
                  Drive Home Your Next Maruti Suzuki
                </h2>
                <p className="text-xs sm:text-sm text-gray-200 line-clamp-2">
                  Hatchbacks, SUVs, sedans and our electric range are all on display today, with product specialists on the floor to help you choose.
                </p>

                <div className="pt-3 flex flex-wrap items-center gap-3">
                  <div className="flex items-center gap-2 text-xs font-semibold text-[#f0d486]">
                    <MapPin className="w-4 h-4 text-[#122B52]" />
                    <span>Available on the Ground Floor Sales Bays</span>
                  </div>
                  <button
                    onClick={() => setShowTestDriveModal(true)}
                    className="px-4 py-2 bg-[#ffd700] hover:bg-[#f0c400] text-[#1a1a1a] text-xs font-bold tracking-wider uppercase rounded-lg transition-colors cursor-pointer flex items-center gap-1.5 shadow-md"
                  >
                    <Car className="w-3.5 h-3.5" />
                    <span>Pre-book a Test Drive</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* Discovery Category Cards Grid (HATCHBACK, SUV, SEDAN, EV) */}
        ...
        <section className="space-y-6 pt-4 border-t border-[#eae4d5]">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-[#555045] mt-1">
                "Browse the model range available at this showroom."
              </p>
            </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
            {['ALL', 'Hatchback', 'SUV', 'Sedan', 'MPV', 'Certified Pre-Owned'].map((filter) => (
              <button
                key={filter}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredBrands.map((brand) => (
              ...
              >
                ...
                {/* VIEW ON SALES FLOOR Button */}
                <div className="pt-4 mt-3 border-t border-[#f0ebd9] flex items-center justify-between">
                  <span className="text-[11px] text-[#888172] font-medium">On Display Today</span>
                  <button
        ...
        <section className="space-y-6">
          <div className="max-w-3xl">
            ...
            <p className="text-sm sm:text-base text-[#555045] mt-2 leading-relaxed">
              "Book a periodic service, check on repair status, or speak with a service advisor."
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="bg-white border border-[#e8e2d5] rounded-2xl p-6 shadow-sm space-y-3">
              <div className="p-3 bg-[#122B52] text-white rounded-xl w-fit">
                <Wrench className="w-5 h-5" />
              </div>
              <h3 className="text-lg font-serif font-bold text-[#1a1a1a]">Book a Service</h3>
              <p className="text-xs text-[#666052] leading-relaxed">
                Schedule a periodic service, repair, or express 90-minute service slot with our team.
              </p>
              <button
                onClick={() => handleOpenFloor(STORE_FLOOR_DIRECTORY.find(f => f.id === 'floor-service') || STORE_FLOOR_DIRECTORY[0])}
                className="px-4 py-2.5 bg-[#122B52] hover:bg-[#0B2038] text-white text-xs font-bold tracking-wider uppercase rounded-lg transition-colors cursor-pointer"
              >
                View Service Center →
              </button>
            </div>
            ...
            <div className="bg-white border border-[#e8e2d5] rounded-2xl p-6 shadow-sm space-y-3">
              ...
              <h3 className="text-lg font-serif font-bold text-[#1a1a1a]">Insurance & Warranty</h3>
              <p className="text-xs text-[#666052] leading-relaxed">
                Cashless claims, new policy issuance, and extended warranty add-ons at the Insurance Desk.
              </p>
              <button
                onClick={() => handleOpenFloor(STORE_FLOOR_DIRECTORY.find(f => f.id === 'floor-insurance') || STORE_FLOOR_DIRECTORY[0])}
                className="px-4 py-2.5 bg-white border border-[#122B52] text-[#122B52] hover:bg-[#122B52] hover:text-white text-xs font-bold tracking-wider uppercase rounded-lg transition-colors cursor-pointer"
              >
                View Insurance Desk →
              </button>
            </div>
        ...
        {/* ==================================================
            SHOWROOM DIRECTORY — STORE DIRECTORY / WAYFINDING
        ================================================== */}
        <section className="space-y-6 pt-4 border-t border-[#eae4d5]">
          <div>
            <div className="text-xs font-bold uppercase tracking-widest text-[#122B52] mb-1">
            ...
            <p className="text-xs sm:text-sm text-[#555045] mt-1">
              "Find the Sales Floor, Service Center, or Insurance Desk without wandering the showroom."
            </p>
          </div>
        ...
        <section className="space-y-6 pt-4 border-t border-[#eae4d5]">
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#555045] mt-1">
              Popular models, service camps, and offers active at this showroom today.
            </p>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#666052] mt-0.5">
                Unlocked through Maruti Suzuki In-Store Wi-Fi • Valid for this store session
              </p>
            </div>
          </div>
      ...
      <footer className="mt-16 bg-[#ffffff] border-t border-[#e8e2d5] py-8 px-4 sm:px-6 lg:px-8 text-center text-xs text-[#777063] space-y-3">
        ...
        <p className="text-[11px] text-[#888172]">
          © 2026 Maruti Suzuki India Limited. Wi-Fi captive portal designed for in-store guest navigation and customer rewards.
        </p>
      </footer>
