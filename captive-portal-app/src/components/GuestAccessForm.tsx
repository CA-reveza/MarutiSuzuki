...
export const GuestAccessForm: React.FC<GuestAccessFormProps> = ({ onSuccessNewUser, onSuccessReturningUser }) => {
  ...
  useEffect(() => {
    const loadStored = async () => {
      ...
      try {
        const local = localStorage.getItem('MARUTI SUZUKI_STORED_CUSTOMERS');
    ...
    setTimeout(() => {
      ...
      if (activeTab === 'returning') {
        ...
        const returningCust: CustomerInfo = {
          ...
          sessionVoucherCode: recognizedMember?.sessionVoucherCode || 'ACCESSORIES20',
          sessionVoucherDiscount: '20% OFF',
          sessionVoucherDesc: 'Flat 20% off on Genuine Accessories & Car Care Products',
          sessionVoucherMinOrder: '₹4,999'
        };

        const returningCoupon = returningCust.sessionVoucherCode || 'ACCESSORIES20';
        ...
        try {
          ...
          localStorage.setItem('marutisuzuki_wifi_telemetry_event', JSON.stringify(eventPayload));
          const channel = new BroadcastChannel('marutisuzuki_wifi_channel');
        ...
        const voucherCode = `ACCESSORIES20`;
        const newCustObj = {
          id: `CUST-${Math.floor(1000 + Math.random() * 9000)}`,
          name: cleanName,
          email: cleanEmail || `${cleanPhone}@marutisuzukis-wifi.in`,
        ...
        try {
          const existingStr = localStorage.getItem('MARUTI SUZUKI_STORED_CUSTOMERS');
          ...
          localStorage.setItem('MARUTI SUZUKI_STORED_CUSTOMERS', JSON.stringify(existingArr));
        } catch (e) {}
          ...
          <div className="p-6 sm:p-8">
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#122B52]/10 text-[#122B52] text-xs font-bold uppercase tracking-wider mb-2">
                ...
                <span>{activeTab === 'new' ? 'Maruti Suzuki Guest Wi-Fi' : 'First Citizen Member Login'}</span>
              </div>
              <img src={marutiSuzukiLogo} alt="Maruti Suzuki" className="h-12 sm:h-14 w-auto mx-auto mb-1" />
              <p className="text-xs text-[#777063] font-semibold tracking-widest -mt-1">MARUTISUZUKI.com</p>
              ...
              <div className="py-10 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#777063] mt-1">
                    Welcome to Maruti Suzuki • {fullName || 'Valued Guest'}
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  {activeTab === 'new' && (
                    <label className="flex items-start gap-2.5 cursor-pointer">
                      ...
                      <span className="text-xs text-[#555045] leading-snug">
                        I want to receive exclusive offers, fashion updates & VIP privileges from Maruti Suzuki.
                      </span>
                    </label>
                  )}
