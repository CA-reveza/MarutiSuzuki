...
export default function App() {
  ...
  const [customer, setCustomer] = useState<CustomerInfo>({
    ...
    sessionVoucherCode: 'ACCESSORIES20',
  ...
  const broadcastTelemetry = (type: string, payload: any) => {
    ...
    try {
      localStorage.setItem('marutisuzuki_wifi_telemetry_event', JSON.stringify(eventData));
    } catch (e) {}
    try {
  ...
  const handleGuestLoginSuccess = (customerData: CustomerInfo) => {
    try {
      sessionStorage.removeItem('marutisuzuki_portal_reviewed');
      sessionStorage.removeItem('marutisuzuki_portal_pulse_checked');
    } catch (e) {}
