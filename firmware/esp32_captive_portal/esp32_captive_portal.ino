...
void setup() {
  ...
  Serial.println("\n\n=============================================");
  Serial.println("  MARUTI SUZUKI Captive Portal — AXIONIK");
  Serial.println("=============================================");
  ...
  bool apOk = WiFi.softAP("MARUTI SUZUKI_WiFi", NULL, 1, 0, 4);
  
  if (apOk) {
    Serial.println(">>> SUCCESS: MARUTI SUZUKI_WiFi IS BROADCASTING! <<<");
  ...
  if (MDNS.begin("marutisuzukis")) {
    Serial.println("mDNS Service: Started (http://marutisuzukis.local)");
...
void loop() {
  ...
  if (millis() - lastHeartbeat > 2000) {
    ...
    Serial.println("[ESP32 HEARTBEAT] MARUTI SUZUKI_WiFi is broadcasting...");
  }
}
